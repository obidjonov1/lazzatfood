...
class Follow {
  ...
  async subscribeData(member, data) {
    try {
      // o'zimizga o'zimiz subscribeni oldini olish ->
      ...
      // follow qilinayotgan user mavjudmi? ->
      ...
      /* FOLLOW and SUBSCRIBER COUNT INC---> */
  ...
  async unsubscribeData(member, data) {
    try {
      ...
      /* FOLLOW and SUBSCRIBER COUNT DECR---> */
      // follow qilinayotgan user ->
      await this.modifyMemberFollowCounts(follow_id, "subcriber_change", -1);
      // follow qilayotgan user ->
      await this.modifyMemberFollowCounts(subscriber_id, "follow_change", -1);
  ...
  async getMemebrFollowingsData(inquiry) {
    try {
  ...
  async getMemebrFollowersData(member, inquiry) {
    try {
      ...
      /* authMember req qilayotgan bo'lsa, hamda req qilyotgan user o'zining 
       followerlar ro'yhatini req qilyotgan bo'lsa --> */
      if (member && member._id === inquiry.mb_id) {
        aggregateQuery.push(lookup_auth_member_following(follow_id, "follows")); // (config.js[28])
      }
